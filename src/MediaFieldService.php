...
use X3P0\MediaData\Contracts\FieldFactory;
...
final class MediaFieldService
{
	...
	public function getField(int $mediaId, string $fieldKey): ?MediaField
	{
		// Get media data from repository
		$media = $this->repository->find($mediaId);

		// If no media found or field is not registered, bail.
		if (! $media || ! $this->registry->isRegistered($fieldKey)) {
			return null;
		}

		// Create field instance.
		$field = $this->factory->make($fieldKey, $media);

		// Return the field if data exists.
		return $field && $field->exists() ? $field : null;
	}
}
